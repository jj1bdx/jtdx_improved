...
subroutine hintdyncq(mrs,mrs2,direction,callsign,grid,decoded,hint_dyn)
  ...
  logical(1) hint_dyn,first
  character direction*2,callsign*6,grid*4,msg*22,msg1*22,msg00*22,decoded*22,prevcall*6
  ...
  if(callsign.eq.prevcall) then
     go to 2
  else
     prevcall=callsign
  endif
  
  if(direction.ne."01") msg='CQ '//direction//' '//callsign//' '//grid
  if(direction.eq."01") msg='CQ '//callsign//' '//grid
  call fmtmsg(msg,iz)
  call packmsg(msg,dgen,itype)            !Pack message into 72 bits
  call rs_encode(dgen,sym_rev)            !RS encode
  call interleave63(sym_rev,1)            !Interleave channel symbols
