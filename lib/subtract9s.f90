...
!  use timer_module, only: timer
  use packjt
  use jt65_mod6 ! real*4 dd(624000) npts9=597888
...
!  call timer('subtr_3 ',0)
  do m=1,nref
     j2=nstart+m-1
     if(j2.ge.1 .and. j2.le.(624000-1)) dd(j2)=dd(j2)-2*REAL(cfilt(m)*cref(m))
  enddo
